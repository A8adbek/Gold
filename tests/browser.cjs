...
(async()=>{
 ...
 try{
   ...
   await page.keyboard.press('Enter');assert.equal(await page.locator('#launch-panel').isVisible(),false,'Enter must launch without reactivating the previously focused restart button');
   await page.clock.runFor(35000);assert.ok(await page.locator('#overlay').isVisible());
